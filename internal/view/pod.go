...
func (p *Pod) bindDangerousKeys(aa ui.KeyActions) {
	aa.Add(ui.KeyActions{
		...
		ui.KeyT:        ui.NewKeyAction("Attach", p.attachCmd, true),
	})
}
...
func (p *Pod) showController(evt *tcell.EventKey) *tcell.EventKey {
	...
	if co != nil {
		...
		if err := p.App().inject(co, false); err != nil {
			p.App().Flash().Err(err)
		}
	}
	return nil
...
func (p *Pod) killCmd(evt *tcell.EventKey) *tcell.EventKey {
	...
	for _, path := range selections {
		if err := nuker.Delete(context.Background(), path, nil, dao.ForceGrace); err != nil {
